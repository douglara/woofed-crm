...
Doorkeeper.configure do
  ...
  # Bridge to Devise: if nobody is signed in, Warden bounces to the sign-in screen and
  # returns here after authentication. Used by /oauth/authorize for the consent flow.
  ...
  #
  force_pkce
  # OAuth 2.1 / MCP spec forbid `plain` — only S256 is accepted.
...
# Map Doorkeeper's token generation onto the exact payload shape produced by
# Users::JsonWebToken.encode_user, so a JWT issued via OAuth and a JWT issued
# manually are indistinguishable to Mcp::JwtAuthenticator.
Doorkeeper::JWT.configure do
  secret_key Rails.application.secrets.secret_key_base.to_s
  signing_method :hs256

  token_payload do |opts|
    # `jti` (JWT ID, RFC 7519 §4.1.7) is a random nonce per token. Without it,
    # two tokens issued in the same second with the same sub+exp would be
    # byte-identical and hit the unique constraint on oauth_access_tokens.token.
