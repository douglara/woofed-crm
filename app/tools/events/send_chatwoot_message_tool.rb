module Events
  class SendChatwootMessageTool < ApplicationTool
    ...
    arguments do
      ...
      required(:app_id).filled(:integer).description('Chatwoot app integration ID')
      required(:chatwoot_inbox_id).filled(:string).description('Target Chatwoot inbox ID')
    ...
    def call(content:, app_id:, chatwoot_inbox_id:, deal_id: nil, contact_id: nil,
             send_now: false, scheduled_at: nil)
