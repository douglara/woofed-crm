class Stages::BroadcastUpdates
  ...
  def self.call(stage, filter_status_deal)
    stage_filtered_deals = stage.deals.where(status: filter_status_deal).order(:position).limit(8)
    stage_all_deals = stage.deals.order(:position).limit(8)

    pagy_filtered_deals = Pagy.new(count: stage.deals.where(status: filter_status_deal).count, items: 8)
    pagy_all_deals = Pagy.new(count: stage.deals.limit(8).count, items: 8)
