...
RSpec.describe Accounts::UsersController, type: :request do
  ...
  describe 'POST /accounts/{account.id}/users' do
    let(:valid_params) { { user: { full_name: 'Yukio', email: 'yukio@email.com', password: '123456', password_confirmation: '123456' } } }
    ...
    context 'when it is an authenticated user' do
      ...
      context 'create user' do
        ...
        context 'when email is invalid' do
            ...
            invalid_params = { user: {full_name: 'Yukio', email: '', password: '123456', password_confirmation: '123456'}}
              expect do
                post "/accounts/#{account.id}/users",
            ...
            invalid_params = { user: {full_name: 'Yukio', email: 'email invalido', password: '123456', password_confirmation: '123456'}}
                expect do
                post "/accounts/#{account.id}/users",
                params: invalid_params
                end.to change(User, :count).by(0)
        ...
        context "when password is invalid" do
            it 'when password is blank' do
                invalid_params = { user: { full_name: 'Yukio', email:'yukio@email.com', password: '', password_confirmation: '' } }
                expect do
                    post "/accounts/#{account.id}/users",
                    params: invalid_params
                end.to change(User, :count).by(0)
            ...
            it 'when password have less 6 characters' do
                invalid_params = { user: { full_name: 'Yukio', email:'yukio@email.com', password: '123', password_confirmation: '123' } }
                expect do
                    post "/accounts/#{account.id}/users",
                    params: invalid_params
                end.to change(User, :count).by(0)
            ...
            it 'when password and password_confirmation is diferent' do
                invalid_params = { user: { full_name: 'Yukio', email:'yukio@email.com', password: '123', password_confirmation: '456' } }
                expect do
                    post "/accounts/#{account.id}/users",
                    params: invalid_params
                end.to change(User, :count).by(0)
        ...
        it "when user exists" do
            invalid_params = { user: {full_name: 'dsada', email: user.email, password: '123456', password_confirmation: '123456' } }
            expect do
                post "/accounts/#{account.id}/users",
                params:  invalid_params
            end.to change(User, :count).by(0)
    ...
    context 'when it is an authenticated user' do
      ...
      context 'get users' do
        ...
        it 'get users by account' do
           ...
           create(:user, full_name: 'Yukio', email: 'yukio@email.com', password: '123456', password_confirmation: '123456', account_id: account_2.id)
           get "/accounts/#{account.id}/users"
           expect(response.body).to include("belchior@show.com.br")
  ...
  describe 'PACTH /accounts/{account.id}/users/{user.id}' do
    ...
    context 'when it is an authenticated user' do
      ...
      context 'update user' do
        let(:valid_params) { { user: { full_name: 'Yukio Updated', email: 'yukio@email.com', password: '123456', password_confirmation: '123456' } } }
        it do
        ...
        it "when password is blank" do
            params = { user: { full_name: 'Yukio Updated', email:'yukio@email.com', password: '', password_confirmation: '' } }
                patch "/accounts/#{account.id}/users/#{user.id}",
                params: params
            expect(User.first.full_name).to eq('Yukio Updated')
