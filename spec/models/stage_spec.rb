...
RSpec.describe Stage do
  context 'scopes' do
    ...
    let(:filter_open) { { status_eq: 'open' } }
    let(:filter_lost) { { status_eq: 'lost' } }

    describe '#total_amount_deals' do
      it 'status open' do
        expect(stage.total_amount_deals(filter_open)).to be 21
      end
      it 'status lost' do
        expect(stage.total_amount_deals(filter_lost)).to be 16
      end
      it 'status is blank' do
        expect(stage.total_amount_deals({})).to be 37
      end
    end

    describe '#total_quantity_deals' do
      it 'status open' do
        expect(stage.total_quantity_deals(filter_open)).to be 3
      end
      it 'status lost' do
        expect(stage.total_quantity_deals(filter_lost)).to be 2
      end
      it 'status is blank' do
        expect(stage.total_quantity_deals({})).to be 5
      end
    end
  end
end
