...
RSpec.describe 'MCP tool: products_list', type: :request do
  ...
  context 'when it is an unauthenticated user' do
    it 'returns unauthorized' do
      post '/mcp/messages', params: mcp_tool_call_body('products_list'),
  ...
  context 'when it is an authenticated user' do
    it 'returns all products when no filter is provided' do
      post '/mcp/messages', params: mcp_tool_call_body('products_list'), headers: auth_headers
    ...
    it 'filters by name, identifier, amount range and custom_attributes' do
      post '/mcp/messages', params: mcp_tool_call_body('products_list', { name: 'cheap' }), headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([cheap_product.id])

      post '/mcp/messages', params: mcp_tool_call_body('products_list', { identifier: 'EXP' }), headers: auth_headers
    ...
    it 'filters by created_at range' do
      post '/mcp/messages', params: mcp_tool_call_body('products_list', { created_from: 2.hours.ago.iso8601 }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([expensive_product.id])

      post '/mcp/messages', params: mcp_tool_call_body('products_list', { created_to: 2.hours.ago.iso8601 }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([cheap_product.id])
    end
    ...
    it 'filters by updated_at range' do
      ...
      post '/mcp/messages', params: mcp_tool_call_body('products_list', { updated_from: 2.hours.ago.iso8601 }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([expensive_product.id])

      post '/mcp/messages', params: mcp_tool_call_body('products_list', { updated_to: 2.hours.ago.iso8601 }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([cheap_product.id])
    end
  end
