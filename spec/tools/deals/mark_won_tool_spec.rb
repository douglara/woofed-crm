...
RSpec.describe 'MCP tool: deals_mark_won', type: :request do
  ...
  context 'when it is an unauthenticated user' do
    it 'returns unauthorized' do
      post '/mcp/messages', params: mcp_tool_call_body('deals_mark_won', { id: deal.id }),
  ...
  context 'when it is an authenticated user' do
    it 'marks the deal as won and sets won_at to now by default' do
      freeze_time do
        post '/mcp/messages', params: mcp_tool_call_body('deals_mark_won', { id: deal.id }), headers: auth_headers
    ...
    it 'returns not found when the deal does not exist' do
      post '/mcp/messages', params: mcp_tool_call_body('deals_mark_won', { id: 99_999 }), headers: auth_headers
    ...
    context 'when required arguments are missing' do
      it 'returns a schema validation error when id is missing' do
        post '/mcp/messages', params: mcp_tool_call_body('deals_mark_won', {}), headers: auth_headers
