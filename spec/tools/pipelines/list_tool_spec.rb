...
RSpec.describe 'MCP tool: pipelines_list', type: :request do
  ...
  context 'when it is an unauthenticated user' do
    it 'returns unauthorized' do
      post '/mcp/messages', params: mcp_tool_call_body('pipelines_list'),
  ...
  context 'when it is an authenticated user' do
    it 'returns pipelines with their stages ordered by position' do
      post '/mcp/messages', params: mcp_tool_call_body('pipelines_list'), headers: auth_headers
    ...
    it 'filters pipelines by name and id' do
      post '/mcp/messages', params: mcp_tool_call_body('pipelines_list', { name: 'sales' }), headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([sales.id])

      post '/mcp/messages', params: mcp_tool_call_body('pipelines_list', { id: onboarding.id }), headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([onboarding.id])
    end

    it 'paginates results' do
      post '/mcp/messages', params: mcp_tool_call_body('pipelines_list', { per_page: 1, page: 1 }), headers: auth_headers
