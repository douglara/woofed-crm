...
RSpec.describe 'MCP tool: pipelines_create', type: :request do
  ...
  context 'when it is an unauthenticated user' do
    it 'returns unauthorized' do
      expect do
        post '/mcp/messages', params: mcp_tool_call_body('pipelines_create', { name: 'sales' }),
  ...
  context 'when it is an authenticated user' do
    it 'creates a pipeline with the given name' do
      expect do
        post '/mcp/messages', params: mcp_tool_call_body('pipelines_create', { name: 'sales' }), headers: auth_headers
    ...
    context 'when required arguments are missing' do
      it 'returns a schema validation error when name is missing' do
        expect do
          post '/mcp/messages', params: mcp_tool_call_body('pipelines_create', {}), headers: auth_headers
        end.not_to change(Pipeline, :count)
