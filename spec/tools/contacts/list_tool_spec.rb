...
RSpec.describe 'MCP tool: contacts_list', type: :request do
  ...
  context 'when it is an authenticated user' do
    ...
    it 'filters by partial full_name, email, phone and id' do
      post '/mcp/messages', params: mcp_tool_call_body('contacts_list', { full_name: 'john' }), headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([john.id])

      post '/mcp/messages', params: mcp_tool_call_body('contacts_list', { email: 'other' }), headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([jane.id])

      post '/mcp/messages', params: mcp_tool_call_body('contacts_list', { phone: '999990001' }), headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([john.id])

      post '/mcp/messages', params: mcp_tool_call_body('contacts_list', { id: jane.id }), headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([jane.id])
    end

    it 'filters by created_at range' do
      post '/mcp/messages', params: mcp_tool_call_body('contacts_list', { created_from: 2.hours.ago.iso8601 }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([jane.id])

      post '/mcp/messages', params: mcp_tool_call_body('contacts_list', { created_to: 2.hours.ago.iso8601 }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([john.id])
    end
    ...
    it 'filters by updated_at range' do
      ...
      post '/mcp/messages', params: mcp_tool_call_body('contacts_list', { updated_to: 2.hours.ago.iso8601 }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([john.id])
    end
    ...
    it 'filters by custom_attributes key/value' do
      ...
      post '/mcp/messages', params: mcp_tool_call_body('contacts_list', { custom_attributes: { city: 'RJ' } }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([john.id])
    end

    it 'paginates results' do
      post '/mcp/messages', params: mcp_tool_call_body('contacts_list', { per_page: 1, page: 1 }), headers: auth_headers
