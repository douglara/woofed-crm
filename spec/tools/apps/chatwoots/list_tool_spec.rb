...
RSpec.describe 'MCP tool: apps_chatwoots_list', type: :request do
  ...
  context 'when it is an unauthenticated user' do
    it 'returns unauthorized' do
      post '/mcp/messages', params: mcp_tool_call_body('apps_chatwoots_list'),
  ...
  context 'when it is an authenticated user' do
    it 'returns all chatwoot integrations when no filter is provided' do
      post '/mcp/messages', params: mcp_tool_call_body('apps_chatwoots_list'), headers: auth_headers
    ...
    it 'filters by id, name, status, chatwoot_endpoint_url and chatwoot_account_id' do
      post '/mcp/messages', params: mcp_tool_call_body('apps_chatwoots_list', { id: active_chatwoot.id }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([active_chatwoot.id])

      post '/mcp/messages', params: mcp_tool_call_body('apps_chatwoots_list', { name: 'sales' }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([active_chatwoot.id])

      post '/mcp/messages', params: mcp_tool_call_body('apps_chatwoots_list', { status: 'inactive' }),
                            headers: auth_headers
    ...
    it 'filters by created_at range' do
      post '/mcp/messages', params: mcp_tool_call_body('apps_chatwoots_list', { created_from: 2.hours.ago.iso8601 }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([inactive_chatwoot.id])

      post '/mcp/messages', params: mcp_tool_call_body('apps_chatwoots_list', { created_to: 2.hours.ago.iso8601 }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([active_chatwoot.id])
    end
    ...
    it 'filters by updated_at range' do
      ...
      post '/mcp/messages', params: mcp_tool_call_body('apps_chatwoots_list', { updated_from: 2.hours.ago.iso8601 }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([inactive_chatwoot.id])

      post '/mcp/messages', params: mcp_tool_call_body('apps_chatwoots_list', { updated_to: 2.hours.ago.iso8601 }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([active_chatwoot.id])
    end
  end
