...
RSpec.describe 'MCP tool: apps_evolution_apis_list', type: :request do
  ...
  context 'when it is an unauthenticated user' do
    it 'returns unauthorized' do
      post '/mcp/messages', params: mcp_tool_call_body('apps_evolution_apis_list'),
  ...
  context 'when it is an authenticated user' do
    it 'returns all evolution_api integrations when no filter is provided' do
      post '/mcp/messages', params: mcp_tool_call_body('apps_evolution_apis_list'), headers: auth_headers
    ...
    it 'filters by id, name, phone and connection_status' do
      post '/mcp/messages', params: mcp_tool_call_body('apps_evolution_apis_list', { id: connected_evolution.id }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([connected_evolution.id])

      post '/mcp/messages', params: mcp_tool_call_body('apps_evolution_apis_list', { name: 'main' }),
                            headers: auth_headers
      expect(mcp_result['data'].pluck('id')).to eq([connected_evolution.id])

      post '/mcp/messages', params: mcp_tool_call_body('apps_evolution_apis_list', { phone: '+5511999990002' }),
                            headers: auth_headers
