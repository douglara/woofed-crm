...
RSpec.describe Inertia::Accounts::Apps::Salesforces::SyncRecordsController, type: :request do
  ...
  describe 'POST /accounts/{account.id}/apps/salesforce/sync_records/{id}/retry' do
    context 'when the reason it failed is gone' do
      it 'imports the row from the payload it kept' do
        sync_record = create(:apps_salesforce_sync_records, app: salesforce, status: 'failed',
                                                            error: "Name can't be blank",
                                                            payload: { 'Id' => '001Hn00001AbCdEIAV',
                                                                       'Name' => 'Acme Ltda' })

        post "#{base_url}/sync_records/#{sync_record.id}/retry"

        expect(sync_record.reload).to be_processed
        expect(Company.find_by(name: 'Acme Ltda')).to be_present
        expect(flash[:notice]).to eq(I18n.t('apps.salesforce.sync_records.processed'))
      end
    end
    ...
    context 'when it fails again' do
      it 'says so and keeps the new reason on the row' do
        ...
        expect(flash[:notice]).to eq(I18n.t('apps.salesforce.sync_records.failed'))
      end
    end

    context 'when the row is gone' do
      it 'says so instead of failing' do
        post "#{base_url}/sync_records/0/retry"

        expect(flash[:alert]).to eq(I18n.t('apps.salesforce.sync_records.not_found'))
      end
    end
  end
end
